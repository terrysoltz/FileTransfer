...
class selectFrame(Frame):
    ...
    def upLevel(self):
        if self.selType == 'source':
            self.master.lst_transfer.delete(0, END)
...
Copy Files button to archive the listed files.",
        ...
        self.lbl_transfer = ttk.Label(self, text = 'The following text files have been modified in the last 24 hours:',
    ...
    # that have been modified in the past day and if so, adds them to listbox
    def checkFiles(self):
        # Get current time and source file path
        curTime = time.time()
